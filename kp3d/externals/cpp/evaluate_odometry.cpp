...
void saveStats (vector<errors> err,string dir,string sequence) {
    ...
    // open file
    FILE *fp = fopen((dir + "/" + sequence + "_stats.txt").c_str(),"w");

    // save errors
    float num = err.size();

//    fprintf(fp,"%f %f\n",(t_err/num),(r_err/num));
    // @ rares.ambrus -> changes below to report the numbers that go in the results table directly
    // i.e. values reported RMSE and degrees drift per 100-800m
    fprintf(fp,"%f %f\n",(t_err/num) * 100.0,(r_err/num) *  ( 57.3 * 100));

    // close file
    fclose(fp);
}

bool eval (string kitti_folder, string results_folder, string sequence, Mail* mail) {

    // ground truth and result directories
    cout<<"Evaluating sequence "<<sequence<<endl;
    cout<<"GT data folder: "<<kitti_folder<<". Results folder: "<<results_folder<<endl;
    string gt_dir         = kitti_folder+"/poses";
    string result_dir     = results_folder;
...
//        plotErrorPlots(plot_error_dir,prefix);
        saveStats(total_err,result_dir, sequence);
    }
...
int32_t main (int32_t argc,char *argv[]) {
    ...
    if (argc!=4) {
        ...
        cout << "Usage: ./eval_odometry KITTI_FOLDER RESULTS_FOLDER SEQ" << endl;
        return 1;
    }

    // read arguments
    string kitti_folder = argv[1];
    string results_folder = argv[2];
    ...
    // run evaluation
    bool success = eval(kitti_folder, results_folder, sequence,mail);
